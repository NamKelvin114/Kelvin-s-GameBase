...
{
    public static class NotificationAndroid
    {
        private static readonly Dictionary<string, bool> ChannelRegistered = new();

        private static void RegisterNotificationChannel(string identifier, string name, string description)
        {
            ChannelRegistered.TryGetValue(identifier, out var registered);
